...
export default {
  ...
  methods: {
    ...
    async getFavorites() {
      ...
      await axios
        .get(
          "http://localhost:8081/getFavorites?username=" +
    ...
    async onDel(node) {
      ...
      if (this.token) {
        await axios
          .post("http://localhost:8081/deleteTreeNode", {
            id: node.id,
          })
    ...
    async onChangeName(params) {
      ...
      if (this.token) {
        ...
        if (this.editName == 1) {
          await axios
            .post("http://localhost:8081/updateTreeNodeName", {
    ...
    async onAddNode(params) {
      ...
      if (this.token) {
        ...
        let outer = this;
        await axios
          .post("http://localhost:8081/addTreeNode", {
