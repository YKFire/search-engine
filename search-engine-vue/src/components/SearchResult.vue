          ...
          <div class="input" style="text-align: left">
            <el-autocomplete v-model="search_word" style="width: 600px;"
              ...
              <template v-slot:suffix>
                <el-upload
                    :limit="1"
                    action="http://localhost:8081/imageUpload"
  ...
  methods: {
    ...
    async checkToken() {
      ...
      if (jwt != null) {
        await axios
          .get(
            "http://localhost:8081/survival?token=" +
              jwt.token +
              "&username=" +
              jwt.username
          )
    ...
    async logout() {
      ...
      if (jwt != null) {
        await axios
          .get(
            "http://localhost:8081/user/logout?username=" +
              jwt.username +
              "&token=" +
              jwt.token
          )
    ...
    async search() {
      ...
      await axios
        .get(
          "http://localhost:8081/search_use_split?word=" +
            outer.search_word +
            "&pageNum=" +
            outer.pageNum
        )
    ...
    async getRelatedWord() {
      let outer = this;
      await axios
        .get("http://localhost:8081/related_word?word=" + outer.search_word)
        .then((response) => (outer.info = response.data));
    ...
    async querySearchAsync(queryString, cb) {
      ...
      if (queryString == '') {
        ...
        await axios
            .get("http://localhost:8081/prefix_word?word=" + this.search_word)
            .then((response) => {
              data = response.data
            })
